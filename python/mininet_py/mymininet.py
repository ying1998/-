...
def myNet(cname='controller',cargs='-v ptcp:'):
    ...
    switch0.cmd( 'ovs-vsctl del-br dp1' )
    ...
    for intf in switch1.intfs.values():
        print(intf)
        print(switch1.cmd( 'ovs-vsctl add-prt dp1 %s' % intf))
    ...
    switch1.cmd( 'ovs-ofctl add-flow dp0 \"in_port=1 actions=output:2\"')
    switch1.cmd( 'ovs-ofctl add-flow dp0 \"in_port=2 actions=output:1\"')
